...
\begin{enumerate}
    \item \textbf{Generalizable Detection:} LLMs can be fine-tuned to detect steering with 95.3\% accuracy on concepts absent from the training set (\cref{subsec:res_detection}).
...
\begin{itemize}
    ...
    \item \textbf{Qwen 2.5 32B} \citep{qwen2.5}: Layer 43 of 64 (67\% depth)
...
\begin{table}[h]
\centering
\caption{Detection performance on held-out concepts. Base models show near-zero detection capability, while fine-tuned models achieve up to 95.3\% detection with 0\% false positive rate.}
\label{tab:detection_accuracy}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Base Det.} & \textbf{Adapted Det.} & \textbf{Concept ID} & \textbf{FPR} \\
\midrule
Qwen 2.5 32B & 7.9\% & \textbf{95.3\%} & \textbf{71.2\%} & 0.0\% \\
Gemma 2 9B & 0.0\% & 91.3\% & 78.2\% & 0.0\% \\
...
\midrule
Qwen 32B base & $\sim$83\% & $\sim$90\% \\
Qwen 32B adapted & 79.1\% (--5\%) & 52.1\% (--42\%) \\
\bottomrule
\end{tabular}
\end{table}

Gemma shows severe degradation, particularly on GSM8K (--84\%), while Qwen models exhibit more graceful degradation. Notably, Qwen 32B retains 79.1\% MMLU (--5\% from base) while achieving the highest detection rate (95.3\%), suggesting that larger models may better preserve capabilities during detection training.
...
\begin{itemize}
    ...
    \item Our evaluation is limited to open-source models up to 32B parameters.
\end{itemize}
